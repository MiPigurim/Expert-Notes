          ...
          <Dialog.Content className='fixed overflow-hidden left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col outline-none'>
            ...
            <Dialog.Close className='absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100'>
              ...
              <form className='flex-1 flex flex-col'>
                ...
                {isRecording ? (
                  <button type='button' onClick={handleStopRecording} className='w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center text-sm taxt-slate-300 outline-none font-medium hover:slate-100'>
                    <div className='size-3 rounded-full bg-red-500 animate-pulse'></div>
                    Gravando! (clique para interromper)
                  </button>
                ) : (
                  <button type='button' onClick={handleSaveNote} className='w-full bg-lime-400 py-4 text-center text-sm taxt-lime-950 outline-none font-medium hover:bg-lime-500'>
                    Salvar nota
                  </button>
                )
                 }
